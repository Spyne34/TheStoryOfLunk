...
void USkillManagerComponent::OnSkillRequested(const int32 _skillIndex)
{
	if (_skillIndex >= 0 && _skillIndex < Skills.Num() && Skills[_skillIndex] != nullptr)
	{
		...
		if (!IsASkillExecuting())
		{
			...
			//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("bSkillExecution : %d"), bSkillExecution));
		}
		else
		{
			//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("A Skill is executing")));
		}
	}
}
...
ACharacter* USkillManagerComponent::GetOwnerCharacter()
{
